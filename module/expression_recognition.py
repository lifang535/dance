...
class ExpressionRecognition(Process):
    def __init__(self, 
                 config: dict,
        ...
        # from age_recognition module
        self.age2expression_queue = age2expression_queue
        # to frame_to_video module
        self.expression_queue = expression_queue
        
        self.frame_size = config['frame_size']
        
        self.device = torch.device(config['expression_recognition']['device'])
        self.image_processor_path = config['expression_recognition']['image_processor_path']
        self.model_path = config['expression_recognition']['model_path']
    ...
    def run(self):
        print(f"[ExpressionRecognition] Start!")
        ...
        while not self.end_flag:
            try:
                request = self.age2expression_queue.get(timeout=1)
            except Empty:
                continue
    ...
    def _infer(self, request):
        if request.box is not None:
            ...
            expression = self.id2label[preds.item()]
            
            label = expression
            
            request.label += f" {label}"
        
        self.expression_queue.put(request)
            
    def _end(self):
        self.expression_queue.put(None)
        
        self.end_flag = True
        print(f"[ExpressionRecognition] Stop!")
