...
class ObjectDetection(Process):
    ...
    def _infer(self, request):
        ...
        for i, result in enumerate(results):
            posture_number = sum([1 for label in result["labels"] if self.id2label[label.item()] == 'person'])
            person_number = sum([1 for label in result["labels"] if self.id2label[label.item()] == 'person'])
            
            # print(f"[ObjectDetection] posture_number = {posture_number}, person_number = {person_number}")
            
            request.posture_number = posture_number
            request.person_number = person_number
            
            self.posture_queue.put(request)
            ...
            for score, label, box in zip(result["scores"], result["labels"], result["boxes"]):
                ...
                if self.id2label[label.item()] == 'person': # TODO: modify this condition
                    req = request.copy()
                    req.posture_id = posture_id
                    
                    req.box = box
                    req.label = self.id2label[label.item()]
                    self.posture_queue.put(req)
                    
                    posture_id += 1
                if self.id2label[label.item()] == 'person':
                    req = request.copy()
