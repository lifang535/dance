...
class FrameToVideo(Process):
    def __init__(self, 
        ...
        # posture drawer
        self.posture_image_processor_path = config['posture_recognition']['image_processor_path']
        self.posture_image_processor = None
    ...
    def run(self):
        ...
        self.posture_image_processor = torch.load(self.posture_image_processor_path)
        
        posture_get_thread = Thread(target=self.posture_get)
    ...
    def posture_get(self):
        while not self.end_flag:
            try:
                request = self.posture_frame_queue.get(timeout=1)
            except Empty:
                continue
            ...
            # print(f"[FrameToVideo] video_id: {request.video_id}, frame_id: {request.frame_id}, posture_id: {request.posture_id}")
            
            video_id = request.video_id
            frame_id = request.frame_id
            posture_id = request.posture_id
            ...
            if frame_id not in self.video_dict[video_id]:
                ...
                self.video_dict[video_id][frame_id]['posture'] = {}
                self.video_dict[video_id][frame_id]['person'] = {}
            
            if request.box is not None:
                self.video_dict[video_id][frame_id]['posture'][posture_id] = {'box': request.box, 'candidate': request.candidate, 'subset': request.subset} 
    ...
    # Check if the video contains all the frames, and if the frames contain all the postures and persons
    ...
    def check_video(self, video_id):
        ...
        for frame_id in range(frame_number):
            ...
            posture_number = request.posture_number
            person_number = request.person_number
            
            if len(self.video_dict[video_id][frame_id]['posture']) != posture_number:
                return
            ...
            if frame_id not in self.process_time_dict:
                ...
                self.posture_number[frame_id] = posture_number
    ...
    def save_video(self, video_id):
        ...
        print(f"[FrameToVideo] Video fps: {video_fps}")
        ...
        for frame_id in range(len(self.video_dict[video_id])):
            ...
            for posture_id, posture in self.video_dict[video_id][frame_id]['posture'].items():
                box = posture['box']
                candidate = posture['candidate']
                subset = posture['subset']
                ...
                # print(f"[FrameToVideo] posture box: ({x1}, {y1}, {x2}, {y2}) label {label}")
                
                # cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 0, 255), 2)
                # cv2.putText(frame, label, (x1, y1), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                frame = self.posture_image_processor(frame, candidate, subset, x1, y1)
    ...
    def draw_latency(self): # add，会调用两次
        ...
        # posture_number_list = [self.posture_number[frame_id] for frame_id in range(len(self.posture_number))]
        ...
        # 绘制第二个折线图
        axs[1].plot(person_number_list, label='posture Number', color='orange')
        axs[1].set_title('posture Number Over Frames')
        axs[1].set_xlabel('Frame')
        axs[1].set_ylabel('posture Number')
        axs[1].legend()
